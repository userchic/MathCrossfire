...
namespace MathCrossfire.Repositories
{
    public class GameRepository :IGameRepository
    {
        ...
        {
            return _context.Games.Where(x => x.ID == id).Include(x=>x.Teams).ThenInclude(x=>x.Users).Include(x=>x.Tasks).ThenInclude(x=>x.UsersAnswers).ThenInclude(x=>x.Shot).First();
        }
